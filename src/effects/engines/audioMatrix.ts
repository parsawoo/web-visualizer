...
export function init(root: HTMLElement): () => void {
  ...
  const sourceVideo = $('sourceVideo') as HTMLVideoElement, sourceImage = $('sourceImage') as HTMLImageElement;
  ...
  const FPS = 30;
  let renderer: any, scene: any, camera: any, particleSystem: any, geometry: any, material: any, sourceTex: any = null;
  let renderVideo: HTMLVideoElement | null = null, renderImage: HTMLImageElement | null = null, isVideoMode = false;
  let renderAudio: HTMLAudioElement | null = null, bakedAudioData: any[] = [];
  let isPlaying = false, visualFileUrl: string | null = null, rawAudioFile: File | null = null;
  let rafId = 0;

  const exporter = new MediaExporter({ getCanvas: () => webglCanvas, getAudioElement: () => renderAudio, baseName: 'AudioMatrix', fps: FPS });
  ...
  function syncCameraAspect() {
    ...
    camera.aspect = mediaAspect; camera.updateProjectionMatrix();
  }
  ...
  $('visualUpload').onchange = (e: any) => {
    ...
    if (file.type.startsWith('video/')) {
      isVideoMode = true; sourceVideo.src = visualFileUrl; sourceVideo.style.display = 'block';
      sourceVideo.onloadeddata = () => { syncCameraAspect(); checkReady(); };
    } else {
      isVideoMode = false; sourceImage.src = visualFileUrl; sourceImage.style.display = 'block';
      sourceImage.onload = () => { syncCameraAspect(); checkReady(); };
    }
  };
  $('audioUpload').onchange = (e: any) => { rawAudioFile = e.target.files[0]; checkReady(); };
  ...
  btnMake.onclick = async () => {
    ...
    try {
      ...
      bakedAudioData = await bakeAudio(audioBuffer, { fps: FPS, mode: 'bands', onProgress: (r) => (progressBar.style.width = `${r * 100}%`) });
      ...
      progressContainer.style.display = 'none'; loadingSpinner.style.display = 'none'; statusOverlay.style.display = 'none';
      btnPlay.disabled = false; btnRecord.disabled = false; btnMake.textContent = 'Matrix Ready';
    } catch (err) { console.error(err); statusText.textContent = '오디오 분석에 실패했습니다.'; btnMake.disabled = false; loadingSpinner.style.display = 'none'; }
  };

  function setupRenderTexture(): Promise<void> {
    return new Promise((resolve) => {
      if (isVideoMode) {
        ...
        renderVideo.onloadeddata = () => {
          ...
          syncCameraAspect(); buildParticleSystem(renderVideo!.videoWidth, renderVideo!.videoHeight); resolve();
        };
        ...
        renderImage.onload = () => {
          ...
          syncCameraAspect(); buildParticleSystem(renderImage!.width, renderImage!.height); resolve();
        };
      }
    });
  }

  function buildParticleSystem(origW: number, origH: number) {
    if (particleSystem) scene.remove(particleSystem);
    const aspect = origW / origH, res = parseInt(uiResolution.value), w = res, h = Math.floor(res / aspect);
    geometry = new THREE.BufferGeometry();
    const positions = new Float32Array(w * h * 3), uvs = new Float32Array(w * h * 2);
    let idx = 0, uvIdx = 0;
    for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
      positions[idx++] = (x / w - 0.5) * 16.0 * aspect; positions[idx++] = (y / h - 0.5) * 16.0; positions[idx++] = 0;
    ...
    material = new THREE.ShaderMaterial({
      uniforms: {
        tDiffuse: { value: sourceTex }, uAudio: { value: new THREE.Vector3(0, 0, 0) }, uTime: { value: 0.0 },
        uSensitivity: { value: 1.5 }, uDispersion: { value: 2.0 }, uPointSize: { value: 3.0 }, uColorMode: { value: 0 }, uReactMode: { value: 0 },
      },
      vertexShader: `
        varying vec3 vColor;
        ...
        void main() {
          vec4 texColor = texture2D(tDiffuse, uv); vColor = texColor.rgb;
          float lum = dot(texColor.rgb, vec3(0.299,0.587,0.114)); vec3 pos = position;
          float bass = uAudio.x, mid = uAudio.y, treble = uAudio.z;
          float zForce = lum * bass * uDispersion * 2.5; float n = rand(uv + uTime * 0.1);
      ...
      fragmentShader: `
        ...
        void main() {
          vec2 cc = 2.0 * gl_PointCoord - 1.0; if (dot(cc, cc) > 1.0) discard;
          float lum = dot(vColor, vec3(0.299,0.587,0.114)); vec3 baseColor = vColor;
          float bass = uAudio.x, mid = uAudio.y, treble = uAudio.z;
          ...
          if (uReactMode == 0) {
            float e = (bass*0.6)+(mid*0.3)+(treble*0.1); float b = clamp(0.8 + e*1.8, 0.4, 2.2);
            finalColor *= b; finalColor += vec3(treble*0.25); finalColor = clamp(finalColor, 0.05, 0.95);
          } else {
  ...
  function initGL() {
    ...
    camera = new THREE.PerspectiveCamera(45, box.width / box.height, 0.1, 100); camera.position.z = 18;
  ...
  btnPlay.onclick = () => { if (!renderAudio || bakedAudioData.length === 0) return; if (isPlaying) pause(); else play(); };
  function play() { renderAudio!.play(); if (isVideoMode && renderVideo) renderVideo.play(); isPlaying = true; btnPlay.textContent = 'Pause'; btnPlay.classList.add('playing'); }
  function pause() { renderAudio!.pause(); if (isVideoMode && renderVideo) renderVideo.pause(); isPlaying = false; btnPlay.textContent = 'Play'; btnPlay.classList.remove('playing'); }
  function onEnded() { pause(); if (exporter.recording) { exporter.stop(); resetRecordBtn(); } }
  ...
  function animate(time: number) {
    ...
    } else { visCtx.clearRect(0, 0, visCanvas.width, visCanvas.height); }
    if (material) {
  ...
  btnRecord.onclick = () => {
    ...
    renderAudio!.currentTime = 0; if (isVideoMode && renderVideo) renderVideo.currentTime = 0;
    if (!isPlaying) play();
