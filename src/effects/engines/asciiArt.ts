...
export function init(root: HTMLElement): () => void {
  ...
  let currentSource: any = null, isVideoMode = false;
  let visualFileUrl: string | null = null, rawAudioFile: File | null = null;
  let renderAudio: HTMLAudioElement | null = null, bands: any[] = [];
  let isPlaying = false, RW = 0, RH = 0, asciiAmount = 0, smBass = 0, smMid = 0, smTreble = 0;
  let rafId = 0;

  const exporter = new MediaExporter({ getCanvas: () => canvas, getAudioElement: () => renderAudio, baseName: 'AsciiArt', fps: FPS });
  ...
  function computeRenderSize() {
    ...
    canvas.width = RW; canvas.height = RH; layer.width = RW; layer.height = RH;
    fitCss(); return true;
  }
  function fitCss() {
    if (!RW) return;
    const box = $('renderArea').getBoundingClientRect();
    const aspect = RW / RH; let w = box.width, h = box.height;
    if (w / h > aspect) w = h * aspect; else h = w / aspect;
    canvas.style.width = Math.floor(w) + 'px'; canvas.style.height = Math.floor(h) + 'px';
  }
  const onResize = () => fitCss();
  ...
  btnPlay.onclick = () => { if (!renderAudio || bands.length === 0) return; if (isPlaying) pause(); else play(); };
  function play() {
    if (renderAudio!.currentTime >= renderAudio!.duration - 0.1) renderAudio!.currentTime = 0;
    renderAudio!.play(); if (isVideoMode) sourceVideo.play();
    isPlaying = true; btnPlay.textContent = 'Pause'; btnPlay.classList.add('playing');
  }
  function pause() {
    renderAudio!.pause(); if (isVideoMode) sourceVideo.pause();
  ...
  function charColor(r: number, g: number, b: number, lume: number, mode: string, whiteBg: boolean, glow: number) {
  ...
  function renderFrame() {
    ...
    const cols = Math.max(1, Math.floor(RW / cell)), rows = Math.max(1, Math.floor(RH / cell));
    if (sampleCanvas.width !== cols || sampleCanvas.height !== rows) { sampleCanvas.width = cols; sampleCanvas.height = rows; }
    try { sampleCtx.drawImage(currentSource, 0, 0, cols, rows); } catch { return; }
    const data = sampleCtx.getImageData(0, 0, cols, rows).data;
    const ramp = RAMPS[uiCharset.value] || RAMPS.standard, rampMax = ramp.length - 1;
    ...
    layerCtx.font = `${cell}px 'Space Mono', monospace`; layerCtx.textAlign = 'center'; layerCtx.textBaseline = 'middle';
    ...
    for (let y = 0; y < rows; y++) for (let x = 0; x < cols; x++) {
      ...
      const idx = whiteBg ? Math.round(((255 - lume) / 255) * rampMax) : Math.round((lume / 255) * rampMax);
      const ch = ramp[idx] || ' '; if (ch === ' ') continue;
      layerCtx.fillStyle = charColor(r, g, b, lume, mode, whiteBg, bassGlow);
      const ox = jitter ? (Math.random() - 0.5) * jitter : 0, oy = jitter ? (Math.random() - 0.5) * jitter : 0;
      layerCtx.fillText(ch, x * cell + cell / 2 + ox, y * cell + cell / 2 + oy);
    }

    ctx.clearRect(0, 0, RW, RH);
    try { ctx.drawImage(currentSource, 0, 0, RW, RH); } catch {}
    ctx.globalAlpha = asciiAmount; ctx.drawImage(layer, 0, 0); ctx.globalAlpha = 1;
  }
  ...
  function animate() {
    ...
    smBass += (bass - smBass) * 0.3; smMid += (mid - smMid) * 0.3; smTreble += (treble - smTreble) * 0.3;
    const band = uiBand.value;
    const level = band === 'volume' ? smMid : band === 'treble' ? smTreble : (smMid * 0.6 + smTreble * 0.7);
    const baseAmt = parseFloat(uiAmount.value), react = parseFloat(uiReact.value);
    const target = Math.max(0, Math.min(1, baseAmt + react * level));
  ...
  btnFrame.onclick = () => {
    if (!RW) return;
    const a = document.createElement('a'); a.href = canvas.toDataURL('image/png'); a.download = 'AsciiArt_frame.png'; a.click();
  };

  btnRecord.onclick = () => {
    if (bands.length === 0) return;
    if (exporter.recording) { exporter.stop(); resetRecordBtn(); return; }
    renderAudio!.currentTime = 0; if (isVideoMode) sourceVideo.currentTime = 0;
    if (!isPlaying) play();
