...
let renderVideo = null, selfieSegmentation, renderAudio, bakedAudioData = [];
let isPlaying = false, visualFileUrl = null, rawAudioFile = null, segPending = false;
let currentTargetValue = 0;

const exporter = new MediaExporter({
  getCanvas: () => webglCanvas,
  getAudioElement: () => renderAudio,
  baseName: 'NeuralGhost',
  fps: FPS,
});
...
function syncCameraAspect() {
  ...
  const origW = v.videoWidth || 1920, origH = v.videoHeight || 1080;
...
function initGL() {
  ...
  material = new THREE.ShaderMaterial({
    uniforms: {
      tDiffuse: { value: dummyTex }, tMask: { value: maskTex },
      uTarget: { value: 0.0 }, uTime: { value: 0 },
...
btnMake.onclick = async () => {
  ...
  statusText.textContent = "오디오 반응 에너지 추출 중…";
...
function animate(time) {
  ...
  if (renderVideo && renderVideo.readyState >= 2 && !sourceTex) {
    ...
    sourceTex.minFilter = THREE.LinearFilter; sourceTex.generateMipmaps = false;
  ...
  if (isPlaying && renderAudio) {
    ...
    if (bakedAudioData[frame] !== undefined) {
      const rawTarget = bakedAudioData[frame];
      const k = rawTarget > currentTargetValue ? 0.6 : 0.4;
      ...
      visCtx.fillStyle = '#19e3ff';
      visCtx.fillRect(10, visCanvas.height - currentTargetValue * 26, visCanvas.width - 20, currentTargetValue * 26);
    }
  }
  if (material) {
...
audioUpload.onchange = (e) => { rawAudioFile = e.target.files[0]; btnMake.disabled = !visualFileUrl; };
