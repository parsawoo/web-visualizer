...
const FPS = 30;
let renderer, scene, camera, mesh, material, sourceTex = null;
let isPlaying = false, isAnalyzing = false;
let trackingData = [];
let rawAudioFile = null, renderAudio = null, bakedAudioData = [];
let pulse = 0, hudPulse = 0;

const mergeCanvas = document.createElement('canvas');
const mergeCtx = mergeCanvas.getContext('2d');

const MAX_OBJECTS = 50, MAX_FACES = 40, TOTAL_BOXES = MAX_OBJECTS + MAX_FACES;
let renderBoxes = Array.from({ length: TOTAL_BOXES }, () => ({ active: false, class: '', x: 0, y: 0, w: 0, h: 0 }));
let objectModel, holisticModel, holisticResolve = null;

const exporter = new MediaExporter({
  getCanvas: () => mergeCanvas,
  getAudioElement: () => renderAudio,
  baseName: 'CyberTracker',
  fps: FPS,
});
...
async function initDualAI() {
  try {
    ...
    holisticModel.onResults((r) => { if (holisticResolve) { holisticResolve(r); holisticResolve = null; } });
...
/* ---------- shader (with uPulse audio reaction) ---------- */
const cyberShader = {
  uniforms: {
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      // subtle global beat reaction (whole frame)
...
function syncCameraAspect() {
  ...
  const origW = videoElement.videoWidth || 1920, origH = videoElement.videoHeight || 1080;
  ...
  hudCanvas.width = origW; hudCanvas.height = origH;
  hudCanvas.style.width = uiW + 'px'; hudCanvas.style.height = uiH + 'px';
...
audioUpload.onchange = (e) => { rawAudioFile = e.target.files[0] || null; };
...
btnAnalyze.onclick = async () => {
  ...
  for (let i = 0; i <= totalFrames; i++) {
    ...
    const parsedObjs = objects.map(o => ({
    ...
    if (holisticRes && holisticRes.faceLandmarks) {
      ...
      for (const [name, indices] of Object.entries(features)) {
        let minX = 1, minY = 1, maxX = 0, maxY = 0, valid = false;
        indices.forEach(idx => { if (face[idx]) { valid = true; minX = Math.min(minX, face[idx].x); minY = Math.min(minY, face[idx].y); maxX = Math.max(maxX, face[idx].x); maxY = Math.max(maxY, face[idx].y); } });
        if (valid) {
          let w = Math.max((maxX - minX) * 1.5, 0.03), h = Math.max((maxY - minY) * 1.5, 0.03);
          parsedFaces.push({ class: name, x: (minX + maxX) / 2 - w / 2, y: (minY + maxY) / 2 - h / 2, w, h });
        }
      }
    }
  ...
  btnAnalyze.textContent = "Analysis Done"; btnPlay.disabled = false;
};
...
function drawHUD() {
  ...
  const color = uiColor.value, boxStyle = uiBoxStyle.value, lineStyle = uiLineStyle.value;
  const lineDens = parseFloat(uiLineDensity.value);

  hudCtx.strokeStyle = color;
  hudCtx.lineWidth = 1.2 + hudPulse * 2.2;
  hudCtx.font = `12px 'Space Mono', monospace`;
  hudCtx.shadowBlur = (boxStyle === 'glow' ? 15 : 0) + hudPulse * 22;
  hudCtx.shadowColor = color;

  const activeCenters = [];
  renderBoxes.forEach((rb, i) => {
    if (!rb.active) return;
    const cx = rb.x * hudCanvas.width, cy = rb.y * hudCanvas.height;
    const cw = rb.w * hudCanvas.width, ch = rb.h * hudCanvas.height;
    ...
    if (boxStyle === 'scope') {
      ...
      hudCtx.stroke();
      hudCtx.fillStyle = color; hudCtx.fillText(`[${rb.class.toUpperCase()}]`, cx, cy - 8);
    ...
    if (hudPulse > 0.03) {
      ...
      hudCtx.save();
      hudCtx.globalAlpha = Math.min(0.6, hudPulse * 0.9);
      hudCtx.setLineDash([]); hudCtx.lineWidth = 1 + hudPulse * 2;
      hudCtx.strokeRect(cx - pad, cy - pad, cw + pad * 2, ch + pad * 2);
      hudCtx.restore();
    }
  ...
  for (let i = 0; i < activeCenters.length; i++)
    for (let j = i + 1; j < activeCenters.length; j++) {
      const p1 = activeCenters[i], p2 = activeCenters[j];
      const dist = Math.hypot(p1.x - p2.x, p1.y - p2.y);
      if (dist < connectDist && connectDist > 0) {
        hudCtx.moveTo(p1.x, p1.y);
        if (lineStyle === 'curved') {
          const cpX = (p1.x + p2.x) / 2, cpY = Math.min(p1.y, p2.y) - dist * 0.2;
...
function animate(time) {
  ...
  if (isPlaying && bakedAudioData.length) {
    ...
    const raw = bakedAudioData[frame] || 0;
    pulse += (raw - pulse) * (raw > pulse ? 0.5 : 0.25);
  } else pulse += (0 - pulse) * 0.2;
  const react = parseFloat(uiAudioReact.value);
  hudPulse = pulse * react;

  // EQ meter
  visCtx.clearRect(0, 0, visCanvas.width, visCanvas.height);
  if (bakedAudioData.length) {
    visCtx.fillStyle = '#ff2d6b';
    visCtx.fillRect(10, visCanvas.height - hudPulse * 26, visCanvas.width - 20, hudPulse * 26);
  }
...
/* ---------- record ---------- */
btnRecord.onclick = () => {
  if (trackingData.length === 0) return alert("먼저 [Analyze]로 분석을 완료하세요.");
